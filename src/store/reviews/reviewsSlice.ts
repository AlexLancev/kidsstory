...
export interface ReviewsState extends StateProps {
  reviewsArray: ReviewsType[] | null;
}

export const getReviews = createAsyncThunk<ReviewsType[], void, { rejectValue: ErrorResponse }>(
  'GET_REVIEWS',
  async (_, thunkAPI) => {
    try {
...
const reviewsSlice = createSlice({
  ...
  extraReducers: (builder) => {
    builder
      ...
      .addCase(getReviews.fulfilled, (state, action: PayloadAction<ReviewsType[]>) => {
        state.isLoading = false;
        state.reviewsArray = action.payload;
      })
      .addCase(getReviews.rejected, (state, action: PayloadAction<ErrorResponse | undefined>) => {
        state.isError = true;
        state.isLoading = false;
